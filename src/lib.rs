...
sol_storage! {
    ...
    pub struct Song {
        ...
        mapping(uint8 => StorageVec<StorageString>) usersvec;
        mapping(address => StorageVec<StorageString>) txns;
        mapping(address => StorageVec<StorageString>) songs; 
    }
}
...
impl Song {
    pub fn buy_song(
        ...
        seller: Address, seller_song_data: String, update_seller_data: String,
        song_id: String, sender_activity: String, receiver_activity: String, amount: Uint<256, 4>
    ) {
        let mut buyer_accessor = self.users.setter(buyer);
        buyer_accessor.set(update_buyer_data);
        ...
        if song_id != '' {
            ...
            seller_accessor.set(update_seller_data);
    ...
    pub fn change_price(&mut self, user: Address, song_id: u8, update_song_data: String) {
        ...
        if Some(let mut element) = user_song_accessor.get_mut(song_id) {
            element.set_str(&update_song_data);
        }
    }

    pub fn send_token(
        &mut self, sender: Address, receiver: Address, amount: Uint<256, 4>, 
        sender_activity: String, receiver_activity: String
    ) {
        let mut sender_wallet_accessor = self.wallets.setter(sender);
        let sender_tokens = sender_wallet_accessor.get();
        sender_wallet_accessor.set(send_tokens - amount);
    ...
    pub fn create_newuser(&mut self, user: Address, address: String, data: String, zero_id: u8, amount: Uint<256, 4>, activity: String) {
        let mut user_accessor = self.users.setter(user);
        user_accessor.set(data);

        let mut users_accessor = self.usersvec.setter(zero_id);
    ...
    pub fn get_users(&self) -> Vec<String> {
        let users_accessor = self.usersvec.get(0);
